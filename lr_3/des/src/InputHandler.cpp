...
{
    std::cout << "Syntax:\n";
    char keygen[] = "./des keygen [<key_file_path>]\n" \
            "\tIf key file path is not set,\n" \
            "\tfile named \"key\" will be created at the same directory.\n";
    
    char encrypt[] = "./des encrypt <source_file_path> [<destination_file_path>] \
        [key=\"<key_file_path>\"]\n" \
    ...
    char decrypt[] = "./des decrypt <encrypted_file_path> [<decrypted_file_path>] \
        key=<key_file_path>\n" \
...
std::array<char, 8> InputHandler::getKey()
{
    return m_key;
}
