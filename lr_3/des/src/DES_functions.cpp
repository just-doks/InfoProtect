...
std::array<char, 6> expand(std::array<char, 4> RorL)
{
    ...
    for (int i = 0; i < 6; ++i)
    {
        ...
        for (int j = 0; j < 8; ++j)
        {
            int byte = (E[i * 6 + j] - 1) / 8;
            int bit = (E[i * 6 + j] - 1) - (byte * 8);
...
std::array<char, 4> shrink(std::array<char, 8> B)
{
    ...
    for (int i = 0; i < 8; ++i)
    {
        int row = ( ((B[i] >> 4) & 0x2) | (B[i] & 0x1) ); // Строка
        int column = ((B[i] >> 1) & 0xF); // столбец
        SB[i] = S[i][column][row];
    }
    return SB;
}
...
std::array<char, 4> transposition(std::array<char, 4> BS)
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        for (int j= 0; j < 8; ++j)
        {
            int byte = (P[i * 4 + j] - 1) / 8;
            int bit = (P[i * 4 + j] - 1) - (byte * 8);
...
std::array<char, 7> prepareKey(std::array<char, 8> key)
{
    ...
    for (int i = 0; i < 7; ++i)
    {
        for (int j= 0; j < 8; ++j)
        {
            int byte = (G[i * 7 + j] - 1) / 8;
            int bit = (G[i * 7 + j] - 1) - (byte * 8);
...
std::array<char, 7> shiftCD(std::array<char, 7> CD, int shift)
{
    ...
    //int CDbyte = (((i + shift) >= 28) ? (i + shift - 28) : (i + shift)) / 8;
    //int CDbit = (((i + shift) >= 28) ? (i + shift - 28) : (i + shift)) % 8;
